...
public class LevelManager : CacheTransform {
    ...
    void Start() {
        ...
        GameField.startPos = new Vector3((GameData.maxX - GameData.sizeX)/2f*GameData.distanceBetwObject,
            (GameData.maxY - GameData.sizeYVisible)/2f*GameData.distanceBetwObject, 0);
        ...
        foreach (var chance in GameData.parser.chanceObjects) {
    ...
    public bool IsLose() {
        if (GamePlay.LoseLevel()) {
            ...
            if (!GamePlay.onlyOneMoves) {
                PopupsController.Instance.Show(PopupType.NoMoves);
            }
            //EndMoves();
            else {
                Capitulate();
    ...
    private void EndMoves() {
        var obj = Instantiate(Resources.Load<GameObject>("Prefabs/Interface/NotMoves"));
    ...
    private bool TutorialBonusTime() {
        ...
        if (GamePlay.tutorial != null && GamePlay.tutorial.levelIsTutorial(GameData.numberLoadLevel) &&
            GamePlay.tutorial.isBonusTime()) {
    ...
    private void StartOffset() {
        GamePlay.allObjectManager.StartMove(MoveOffset.Down,
            (GameData.sizeY - GameData.sizeYVisible)*GameData.distanceBetwObject);
        WaitStartGame();
    }
